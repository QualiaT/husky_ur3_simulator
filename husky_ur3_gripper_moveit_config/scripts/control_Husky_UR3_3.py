...
def move_base(a,b):             #x,y좌표를 받아와서 그곳으로 platform을 움직이는 코드 theta는 x와 y좌표에 의해 정해짐.(원점기준)
    ...
    while abs(goal.x-x) >0.1 or abs(goal.y-y) >0.1 or abs(goal_angle-theta) >0.05 : #가까의 범위가 0.1이내로 들어오면 break.
        inc_x = goal.x -x
        inc_y = goal.y -y
...
if __name__=='__main__':
    move_base(1,1)
    move_joint(pi*90/180, pi*-130/180 , pi*111/180, pi*-68/180, pi*-90/180, 0) #home pose
    move_joint(pi*90/180, 0, 0, 0, 0, 0) #pick pose
    move_joint(pi*90/180, pi*-130/180 , pi*111/180, pi*-68/180, pi*-90/180, 0) #home pose
